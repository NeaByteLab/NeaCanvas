...
export const ErrorCanvas = {
  ...
  CANVAS_CONTEXT_POOL_FAILED: (layoutName?: string) =>
    layoutName
      ? `[Layout: ${layoutName}] Failed to get canvas context from pool`
      : 'Failed to get canvas context from pool',
  ...
  // ========================================
  // 🛠️ TOOL & OPERATION ERRORS
  // ========================================

  /** Error log for permanently failed operation */
  OPERATION_FAILURE: (shapeName: string, cause?: Error) =>
    `[Shape: ${shapeName}] Operation permanently failed: ${shapeName}. Cause: ${cause ? `${cause.message}` : ''}`,
  ...
  // ========================================
  // 🎭 NEASMART STATE ERRORS
  // ========================================

  /** Error when canvas state JSON parsing fails */
  INVALID_CANVAS_STATE: (stateKey: string) =>
    `[NeaSmart] Invalid canvas state JSON: ${stateKey.substring(0, 100)}${stateKey.length > 100 ? '...' : ''}`,

  /** Error when glow effect JSON parsing fails */
  INVALID_GLOW_STATE: (glowData: string) =>
    `[NeaSmart] Invalid glow effect JSON: ${glowData.substring(0, 50)}${glowData.length > 50 ? '...' : ''}`,

  /** Error when shadow effect JSON parsing fails */
  INVALID_SHADOW_STATE: (shadowData: string) =>
    `[NeaSmart] Invalid shadow effect JSON: ${shadowData.substring(0, 50)}${shadowData.length > 50 ? '...' : ''}`,

  // ========================================
  // 📱 TOUCH & MOBILE ERRORS
  // ========================================

  /** Touch events not supported error */
  TOUCH_NOT_SUPPORTED: (): string =>
    'Touch events are not supported in this environment',
  /** Too many simultaneous touch points error */
  TOUCH_POINTS_EXCEEDED: (maxPoints: number): string =>
    `Maximum touch points (${maxPoints}) exceeded`,
  /** Touch event handler error */
  TOUCH_HANDLER_ERROR: (eventType: string, error: string): string =>
    `Touch handler error for '${eventType}': ${error}`,
  /** Invalid touch coordinates error */
  TOUCH_COORDINATE_INVALID: (x: number, y: number): string =>
    `Invalid touch coordinates: (${x}, ${y})`,
  /** Mobile performance warning */
  MOBILE_PERFORMANCE_WARNING: (): string =>
    'Performance may be reduced on mobile devices with large datasets',
